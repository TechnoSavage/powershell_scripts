function menu {
  [int]$option = Read-Host "`nWhat would you like to do:
  ...
  Else {Write-Host "`nInvalid Option"
        Exit}
}
...
function max_split {
  ...
  $connectors = Read-Host "`nHow many connectors are in the path of the link?"
  If ($mode -eq 1) {
    $connector_loss = 0.2 * $connectors
    $mode_type = 'Single-Mode'
    [int]$wave = Read-Host "`nWhat is the wavelength being used?
    1 - 1310nm
    2 - 1550nm
    Enter the number of your selection "
  ...
  ElseIf ($mode_type -eq 'Multi-Mode' -and $wavelength -eq 1300) {
    $attenuation = 1.0}
  Else {Write-Host "`nSomething went wrong."
    menu}
  $cable_loss = $cable / 1000 * $attenuation
  Write-Host "`nThe loss introduced by the length of cable for the $mode_type $wavelength nm link
    is $cable_loss dB based on $attenuation dB/km fiber attenuation."
  $total_cable_loss = $connector_loss + $cable_loss
  Write-Host "`nThe total connection loss is $total_cable_loss dB."
  $allowable_loss = $link_loss_budget - $total_cable_loss
  Write-Host "`nThe allowable coupler loss for a TAP is a $allowable_loss dB maximum at the monitor port."
  [int]$choice = Read-Host "`nReference which TAP insertion loss values?
  1 - Industry Standard
  2 - Cubro Average
  Enter the number of your selection "
  If ($choice -eq 1) {match_industry $mode_type $allowable_loss}
  ElseIf ($choice -eq 2) {match_cubro $mode_type $allowable_loss}
  Else {Write-Host "`nInvalid input."
    menu}
}
...
function match_industry {
  ...
  Else {Write-Host "`nSomething went wrong."}
  Write-Host "`nThe following split ratios are acceptable for this link $usable."
}
...
function max_cable {
  ...
  If ($mode -notin $valid_input) {
    Write-Host "`n$mode is not a valid input."
    menu}
  $connectors = Read-Host "`nHow many connectors are in the path of the link?"
  If ($mode -eq 1) {
    $connector_loss = 0.2 * $connectors
    ...
    If ($wave -notin $valid_input) {
      Write-Host "`n$wave is not a valid option."
      menu}
    If ($wave -eq 1) {[int]$wavelength = 1310}
    Else {[int]$wavelength = 1550}
  }
  ...
  Else {Write-Host "`n$mode is not a valid input."
    menu}
  Write-Host "`nThe total loss introduced for the $mode_type link by connectors is $connector_loss dB."
  [int]$split = Read-Host "`nWhat is the split ratio of the TAP?
    1 - 50/50
    2 - 60/40
    3 - 70/30
    4 - 80/20
  ...
  If ($split -notin $valid_split) {
    Write-Host "`n$split is not a valid option."
    menu}
...
function cable_by_eth_standard {
  ...
  If ($mode_type -eq 'Multi-Mode') {
    [int]$standard_type = Read-Host "`nWhat is the Ethernet Standard in use?
        1 - OM1-SX
        2 - OM1-LX
        3 - OM2
        4 - OM3
    ...
    If ($standard_type -notin $valid_input) {
      Write-Host "`n$standard_type is not a valid input."
      menu
    }
  ...
  [int]$speed = Read-Host "`nWhat speed is the link?
    1 - 100M
    2 - 1G
    3 - 10G
    4 - 40G
  ...
  If ($speed -notin $valid_input) {
    Write-Host "`n$speed is not a valid input."
    menu
  }
...
function ethernet_table {
  Write-Host "`nEthernet Fiber Standards and max cabling distance:
